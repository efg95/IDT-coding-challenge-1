...
def getSpecificCountry():
    ...
    if location.count() == 0:
        ...
        for x in location:
            history.append(x)
        showSpecificCountry(history, input)

def showSpecificCountry(history, input):
    newInfo = Toplevel()
    newInfo.geometry("500x500")
    newInfo.wm_title("Specific Country Query History")
    S = Scrollbar(newInfo)
    text = Text(newInfo, height=4, width=50)
    S.pack(side=RIGHT, fill = Y)
    text.pack(side=LEFT, fill=Y)
    S.config(command=text.yview)
    text.config(yscrollcommand= S.set)
    text.insert(END, str(input) + ": \n \n")
    for i in history:
        # print(history[i])
        text.insert(END, i["ip address"] + ", " +i ["location"] + ", " + i["city"] + ", " +str(i["date"]) + "\n \n")
